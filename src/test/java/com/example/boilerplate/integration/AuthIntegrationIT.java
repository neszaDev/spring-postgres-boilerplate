...
public class AuthIntegrationIT {
  ...
  void register_login_refresh_and_logout_flow() {
    ...
    ResponseEntity<AuthTokensResponse> regResp = rest.postForEntity(url("/api/v1/auth/register"), register, AuthTokensResponse.class);
    ...
    ResponseEntity<AuthTokensResponse> rot = rest.postForEntity(url("/api/v1/auth/refresh"), refreshReq, AuthTokensResponse.class);
    ...
    ResponseEntity<Void> logoutResp = rest.postForEntity(url("/api/v1/auth/logout"), logoutReq, Void.class);
    assertThat(logoutResp.getStatusCode()).isEqualTo(HttpStatus.NO_CONTENT);

    ResponseEntity<String> afterLogout = rest.postForEntity(url("/api/v1/auth/refresh"), new RefreshTokenRequest(newTokens.refreshToken()), String.class);
